...
class Compactor(config: CompactorConfig = CompactorConfig()) {
  ...
  def compactBlock(block: Block): Block = {
    val globalVarWalker = new ReducerVisitor[Null, List[String]](
      (L1, L2) => L1 ++ L2,
      List[String]()
    ) {
      override def visitVariable(variable: Variable,
                                 passthrough: Null,
                                 global: Boolean): List[String] = {
        if (global)
          variable match {
            case NamedVariable(name) => List(name)
            case PrefixedVariable(prefix, name) =>
              visitExpression(prefix, passthrough)
            case IndexedVariable(prefix, expression) =>
              visitExpression(prefix, passthrough) ++ visitExpression(
                expression,
                passthrough
              )
          } else List[String]()
    ...
    val globalVariables: Set[String] =
      globalVarWalker.visitBlock(block, null).toSet
    ...
    val localVariableReplacementNames =
      Iterator
        ...
        .filter(
          n =>
            !Compactor.reservedNames.contains(n) && !globalVariables.contains(n)
        )

    val localVariableSubVisitor = new LocalVariableSubVisitor(
      config.shortenTrueFalseLiterals
    )
    localVariableSubVisitor.visitBlock(
      block,
      LocalVarSubData(localVariableReplacementNames, mutable.Map())
    )
  }

...
class TreeBuildVisitor[P] extends AbstractVisitor[P, LuaAST] {
  ...
  def visitField(field: Field, passthrough: P): Field = {
    field match {
      case FieldWithoutIndex(value) => FieldWithoutIndex(visitExpression(value, passthrough))
      case FieldByIndex(key, value) => FieldByIndex(visitExpression(key, passthrough), visitExpression(value, passthrough))
      case FieldByName(key, value) => FieldByName(key, visitExpression(value, passthrough))
    }
  }

  def visitVariable(variable: Variable, passthrough: P, global: Boolean = false): Variable =
    variable match {
  ...
  def visitFunctionCall(call: FunctionCall, passthrough: P): FunctionCall =
    call match {
      case FunctionCallWithoutSelf(prefix, arguments) =>
        FunctionCallWithoutSelf(visitPrefixExpression(prefix, passthrough), arguments.map(visitExpression(_, passthrough)))

      case FunctionCallWithSelf(prefix, name, arguments) =>
        FunctionCallWithSelf(visitPrefixExpression(prefix, passthrough), name, arguments.map(visitExpression(_, passthrough)))
    }

  def visitPrefixExpression(prefixExpression: PrefixExpression, passthrough: P): PrefixExpression =
    prefixExpression match {
      ...
      case ExpressionAsPrefix(expr) => ExpressionAsPrefix(visitExpression(expr, passthrough))
    }

  def visitExpression(expression: Expression, passthrough: P): Expression = {
    expression match {
      case operator: BinaryOperator =>
        BinaryOperator.fromLuaToken(operator.token, visitExpression(operator.left, passthrough), visitExpression(operator.right, passthrough))

      case operator: UnaryOperator =>
        UnaryOperator.fromLuaToken(operator.token, visitExpression(operator.subExpression, passthrough))

      case FunctionExpression(functionBody) =>
        FunctionExpression(FunctionBody(functionBody.parameters, visitBlock(functionBody.body, passthrough)))
      ...
      case v@VarArgs => v
      case TableConstructor(fields) => TableConstructor(fields.map(visitField(_, passthrough)))
    }
  }
  ...
  def visitStatement(statement: Statement, passthrough: P): Statement = {
    ...
    statement match {
      ...
      case LocalDeclaration(names, expressionList) =>
        LocalDeclaration(names, expressionList.map(visitExpression(_, passthrough)))

      case lastStatement: LastStatement =>
        lastStatement match {
          case ContinueStatement => ContinueStatement
          case BreakStatement => BreakStatement
          case ReturnStatement(expressions) => ReturnStatement(expressions.map(e => visitExpression(e, passthrough)))
      ...
      case WhileLoop(condition, body) =>
        WhileLoop(visitExpression(condition, passthrough), visitBlock(body, passthrough))

      case RepeatLoop(body, condition) =>
        RepeatLoop(visitBlock(body, passthrough), visitExpression(condition, passthrough))
      ...
      case IfStatement(condition, body, elseIfs, elseBlock) =>
        IfStatement(
          ...
          elseIfs.map(elseIf =>
            ElseIfStatement(visitExpression(elseIf.condition, passthrough), visitBlock(elseIf.body, passthrough))
          ),
  ...
  def visitStatements(statements: List[Statement], passthrough: P): List[Statement] =

...
class GlobalVariableVisitor
    extends ReducerVisitor[Unit, Set[String]]((a, b) => a ++ b, Set.empty) {
  override def visitField(field: Field, passthrough: Unit): Set[String] =
    field match {
      case FieldWithoutIndex(value) => visitExpression(value, passthrough)
      case FieldByIndex(key, value) =>
        visitExpression(key, passthrough) ++ visitExpression(key, passthrough)
      case FieldByName(key, value) => visitExpression(value, passthrough)
    }

  override def visitVariable(variable: Variable,
                             passthrough: Unit,
                             global: Boolean): Set[String] = variable match {
    ...
    case IndexedVariable(prefix, expression) =>
      visitPrefixExpression(prefix, passthrough) ++ visitExpression(
        expression,
        passthrough
      )
  }
  ...
  override def visitExpression(expression: Expression,
    ...
    expression match {
      case FunctionExpression(functionBody) =>
        visitBlock(functionBody.body, passthrough) -- functionBody.parameters.parameters
          .map(_.name)
  ...
  override def visitStatements(statements: List[Statement],
                               passthrough: Unit): List[Set[String]] =
    throw new RuntimeException("This should never be called")
        ...
        case ((localVars, globalVars), b) =>
          b match {
            case GlobalDeclaration(variables, expressionList) =>
              val newVars = variables.flatMap(
                v => visitVariable(v, passthrough)
              ) ++
                expressionList.flatMap(
                  expr => visitExpression(expr, passthrough)
                )
              (localVars, globalVars ++ (newVars.toSet -- localVars))
            case LocalDeclaration(names, expressionList) =>
              val newVars = expressionList.flatMap(
                expr => visitExpression(expr, passthrough)
              )
              (localVars ++ names, globalVars ++ (newVars.toSet -- localVars))
            case ReturnStatement(exprs) =>
              val newVars =
                exprs.flatMap(expr => visitExpression(expr, passthrough))
              (localVars, globalVars ++ (newVars.toSet -- localVars))
            case ContinueStatement | BreakStatement | GotoStatement(_) |
                LabelDeclaration(_) =>
              (localVars, globalVars)
            case call: FunctionCall =>
              (
                localVars,
                globalVars ++ (visitFunctionCall(call, passthrough) -- localVars)
              )
            case DoBlock(body) =>
              (
                localVars,
                globalVars ++ (visitBlock(body, passthrough) -- localVars)
              )
            case WhileLoop(condition, body) =>
              val whileVars = visitExpression(condition, passthrough) ++ visitBlock(
                body,
                passthrough
              )
              (localVars, globalVars ++ (whileVars -- localVars))
            case RepeatLoop(body, condition) =>
              val repeatVars = visitExpression(condition, passthrough) ++ visitBlock(
                body,
                passthrough
              )
            ...
            case IfStatement(condition, body, elseIfs, elseBlock) =>
              ...
              val elseIfVars = elseIfs.flatMap { e =>
                visitExpression(e.condition, passthrough) ++ visitBlock(
                  e.body,
                  passthrough
                )
              }
              val elseVars = elseBlock
                .map(b => visitBlock(b, passthrough))
            ...
            case ForLoop(
              ...
              val stepVars = stepValue
                .map(expr => visitExpression(expr, passthrough))
                .getOrElse(Set.empty)
            ...
            case ForEachLoop(variables, expressions, block) =>
              val expressionVars =
                expressions.flatMap(expr => visitExpression(expr, passthrough))
              val blockVars = visitBlock(block, passthrough)
            ...
            case FunctionDefinition(name, body, true) =>
              ...
              val newLocalVars = (localVars + firstFuncName) ++ body.parameters.parameters
                .map(_.name) ++
            ...
            case FunctionDefinition(name, body, false) =>
              val firstFuncName = name.name.head
              val newLocalVars = localVars ++ body.parameters.parameters.map(
                _.name
              ) ++
                (if (name.withSelf) Set("self") else Set.empty)

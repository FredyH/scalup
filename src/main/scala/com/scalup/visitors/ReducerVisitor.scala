...
class ReducerVisitor[P, R](returnTypeReducer: (R, R) => R, emptyReturnValue: R)
    extends AbstractVisitor[P, R] {
  ...
  def visitFunctionCall(call: FunctionCall, passthrough: P): R =
    call match {
      case FunctionCallWithoutSelf(prefix, arguments) =>
        returnTypeReducer(
          arguments
            .map(e => visitExpression(e, passthrough))
  ...
  def visitStatement(statement: Statement, passthrough: P): R = {
    ...
    statement match {
      case GlobalDeclaration(variables, expressionList) =>
        returnTypeReducer(
          variables
            .map(visitVariable(_, passthrough, true))
            .foldLeft(emptyReturnValue)(returnTypeReducer),
          expressionList
            .map(e => visitExpression(e, passthrough))
            .foldLeft(emptyReturnValue)(returnTypeReducer)
      ...
      case LocalDeclaration(names, expressionList) =>
        expressionList
          .map(visitExpression(_, passthrough))
          .foldLeft(emptyReturnValue)(returnTypeReducer)
      ...
      case IfStatement(condition, body, elseIfs, elseBlock) =>
        List(
          ...
          elseIfs
            ...
            .reduceOption(returnTypeReducer)
            .getOrElse(emptyReturnValue),
  ...
  def visitBlock(block: Block, passthrough: P): R = {
    visitStatements(block.statements, passthrough)
      .reduceOption(returnTypeReducer)
      .getOrElse(emptyReturnValue)
  }

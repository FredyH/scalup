...
object LuaWhitespace {
  implicit val whitespace = { implicit ctx: ParsingRun[_] =>
    ...
    @tailrec def rec(current: Int,
                     state: Int,
                     blockEquals: Int = 0,
                     remainingBlockEquals: Int = 0): ParsingRun[Unit] = {
      if (!input.isReachable(current)) {
        if (state == 0 || state == 1 || state == 5)
          ...
          if (ctx.verboseFailures)
            ctx.setMsg(startIndex, () => Util.literalize("*/"))
          res
        }
      } else {
        ...
        (state: @switch) match {
          ...
          case 6 =>
            (currentChar: @switch) match {
              case '=' =>
                rec(current + 1, state = 6, blockEquals = blockEquals + 1)
